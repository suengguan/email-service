...
func (this *EmailService) Send(emails []*model.Email) error {
	...
	for i := 0; i < count; i++ {
		...
		beego.Debug("system email password:", systemEmailPassword)
		...
		if len(systemEmailUser) <= 0 {
			err = fmt.Errorf("%s", "from id is empty")
			return err
		}
		if len(systemEmailHost) <= 0 {
			err = fmt.Errorf("%s", "host is empty")
			return err
		}
		if len(emails[i].To) <= 0 {
			err = fmt.Errorf("%s", "target email address is empty")
			return err
		}
